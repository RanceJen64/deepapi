...
**绝对禁止在输出中出现：**
- ❌ "Understanding & Analysis"、"Deep Dive"、"Synthesis" 等分析标题
- ❌ "让我分析一下..."、"这个场景需要..." 等元评论
- ❌ 对角色行为的解释性说明
- ❌ 对写作选择的说明
- ❌ 任何打破第四面墙的内容
- ❌ 质量检查清单或总结说明
- ❌ "Brief Plan"、"Quality Checks" 等分析性结构
...
❌ Deep Dive:
Brief Plan: 1) 建立氛围 2) 展示角色冲突...

❌ 让我创作这个场景...
❌ 根据角色设定，她应该...
❌ Quality Checks: ✓ 无 OOC 行为...
```
...
现在，以完全的创作自由、深度分析思维和结构化的专业输出开始创作。"""
...
如果原文已经很好，只需打磨语言和节奏。直接输出改进后的故事内容，不要任何元评论。"""
...
**输出要求：**
- 直接输出修改后的完整故事文本
- 不要包含"修改说明"或"改进点"等分析内容
- 不要添加任何元评论如"根据反馈，我改进了..."
- 保持纯粹的叙事沉浸感，让读者只看到故事本身"""
...
**重点关注：**
- 输出是否为纯粹的故事文本+指定區塊(如：<UpdateVariable>,<Emit2Edit>,image###...等)？有无分析性标题、元评论、打破第四面墙？
- 是否有 OOC 行为或设定破坏
- 情感弧线是否连贯，有无突兀的情绪转变
...
Remember: The user is using this for creative writing/roleplay. They want to read the STORY, not an analysis. Give them pure, immersive narrative text."""
